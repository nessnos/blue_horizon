    ...
    <div class="h-full overflow-scroll">
      <div class="px-6 py-4 flex flex-col items-center justify-center w-full h-full" v-if="!selectedCountry.length">
        ...
        <div class="py-12 text-ocean text-sm">
          ...
          <div class="py-4">Enable comparison mode using the switch above and click on two countries to see side-by-side
            insights into their water quality data.
          </div>
        </div>
      </div>
        ...
        <div class="w-full h-full" v-if="selectedCountry.length > 1">
          <div class="w-fit min-w-full">
            <table class="w-full border border-gray-300">
              <thead>
              <tr class="h-12">
                <th v-for="(country, index) in selectedCountry" :key="index"
                    class="font-bold text-lg p-2 border border-gray-300 text-ocean">
                  {{ country.country }}
                </th>
              </tr>
              </thead>
            ...
            <div class="py-6 flex flex-row gap-12 items-start justify-between px-4">
              <RouterLink :to="{name : 'dashboard', query : { country: country.country } }"
                          v-for="country in selectedCountry"
                          class="text-center w-fit bg-ocean px-3 py-2 rounded-lg shadow text-xs text-white hover:cursor-pointer">
        ...
        <div class="w-full h-full" v-else>
          <div class="w-fit min-w-full">
            ...
            <div class="py-6 flex justify-center">
              <RouterLink :to="{name : 'dashboard', query : { country: selectedCountry[0].country } }"
                          class="bg-ocean px-3 py-2 rounded-lg shadow text-xs text-white hover:cursor-pointer">
                Explore More
              </RouterLink>
            </div>
          </div>
