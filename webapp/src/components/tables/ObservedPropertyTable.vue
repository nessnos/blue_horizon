  ...
  <div class="h-40 w-full">
    <table>
      <thead>
        <tr class="border-b border-gray-200 text-center text-xs">
          <th
            ...
            :key="index"
            :title="header"
            class="w-24 max-w-24 overflow-hidden text-ellipsis whitespace-nowrap px-4 py-2"
          >
            {{ header }}
          </th>
        </tr>
      ...
      <tbody class="h-32 overflow-y-auto">
        <tr
          v-for="(determinand, index) in paginatedDeterminands"
          :key="index"
          class="text-center text-xs"
        >
          <td v-for="(value, key) in determinand" :key="key" class="p-1">
            <div
              :class="[key == 'uom' ? 'w-24 max-w-24' : 'w-32 max-w-32']"
