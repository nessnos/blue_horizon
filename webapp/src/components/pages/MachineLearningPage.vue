<template>
  <div class="flex h-full w-full flex-col items-start justify-start p-12 py-8">
    <div class="flex flex-row items-end gap-3 py-4">
      <SelectOption
        v-if="models.length"
        :rowData="models"
        :label="'Modèles'"
      ...
      <SelectOption
        ...
        :rowData="countries"
        :flag="true"
        @update:selectedData="filterOptions"
      />
      <SelectOption
    ...
    <div
      class="max-w-full"
      v-if="
        selectedModel.parameters && Object.keys(selectedModel.parameters).length
      "
    >
      <ParameterDisclosure
    ...
    <div
      class="relative my-4 flex h-full w-full flex-col overflow-scroll rounded-lg border border-ocean/50 bg-aqua/10 p-3 text-ocean"
    >
  ...
  },
  {
    ...
    parameters: {
      p: {
        ...
        max: 5,
        step: 1,
        label: "Paramètre p (Auto-régressif)",
      },
      d: {
        value: 1,
        type: "slider",
        ...
        max: 2,
        step: 1,
        label: "Paramètre d (Différenciation)",
      },
      q: {
        value: 1,
        type: "slider",
        ...
        max: 5,
        step: 1,
        label: "Paramètre q (Moyenne mobile)",
      },
    },
  },
])
...
const checkPipelinesStatus = async () => {
  const interval = setInterval(async () => {
    try {
      ...
      if (
        pipelines.value.every(
          (pipeline) =>
            pipeline.status === "FAILURE" || pipeline.status === "SUCCESS"
        )
      ) {
...
const runModel = async () => {
  await axios
    .post("/api/ml/run/" + selectedModel.value.technical_name, {
      model: selectedModel.value,
      filter_kwargs: {
        countryCode: selectedCountry.value.code,
        observedPropertyDeterminandLabel: selectedObservedProperty.value.name,
      },
    })
    .then((response) => {
