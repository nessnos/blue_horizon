<template>
  <div v-if="isLoadingData" class="flex h-full w-full items-center justify-center">
    <LoadingWheel />
  </div>
  <div v-else class="grid w-full h-full grid-cols-4 items-start justify-start">
    ...
    <div class="h-full w-full overflow-scroll">
      <div v-if="!selectedCountry.length" class="max-w-80 items-center justify-center mx-auto pt-10 px-2">
        <div v-if="!comparison" class="min-h-56 text-lg text-ocean">
          <span class="font-semibold">Sélectionnez un pays </span>
          <span class="font-normal">pour explorer en détail ses métriques de qualité de l'eau.</span>
        </div>
        <div v-else class="min-h-56 text-lg text-ocean">
          <span class="font-semibold">Sélectionnez deux pays </span>
          <span class="font-normal">pour explorer en détail leurs métriques de qualité de l'eau et les comparer.</span>
        </div>
        <div class="flex flex-col gap-4 pr-4 text-sm">
          <div class="flex w-full flex-row items-end justify-start gap-5">
            ...
            <Switch v-model="comparison" :class="comparison ? 'bg-ocean' : 'bg-gray-300'"
              ...
              <span :class="comparison ? 'translate-x-4' : 'translate-x-0'" aria-hidden="true"
          ...
          <div>
            Activez le mode de comparaison avec le bouton ci-dessus, 
            puis cliquez sur deux pays pour afficher les données de qualité de l'eau côte à côte.
          </div>
        </div>
      </div>
      <div v-if="selectedCountry.length && generalInfo && Object.keys(generalInfo).length > 0" class="w-full">
        <MapTable :selected-country="selectedCountry" :general-info="generalInfo" />
      </div>
    </div>
  </div>
  ...
  <TransitionRoot :show="showPopUp" appear as="template">
    <ErrorPopUp title="Selection Error" message="You can only select up to two countries at a time for
                  ...
                  one." left-label="Deselect All" right-label="Let Me Choose" :left-action="resetComparison"
        ...
        path.addEventListener("click", async function () {
          ...
          if (country) {
            if (country.clicked) {
              ...
              const numberClicked = availableCountries.value.filter((s) => s.clicked).length;
              if (numberClicked == 2) {
                ...
                try {
                  const response = await axios.get(`/api/data/map/general-info?country_code=${country.code}`);
