...
class GetDashBoardData(APIView):
    def post(self, request):
        ...
        if queryset.values().count() > 0:
            yearly_data = queryset.values("reference_year").annotate(avg_value=Avg("result_mean_value")).order_by(
                "reference_year")
            ...
            return Response(
                {
                    'count_chemicals_monitored':
                        queryset.aggregate(Count('observed_property_determinand', distinct=True))[
                            'observed_property_determinand__count'],
                    'count_monitoring_sites': len(set(
                        monitoring_site for sublist in queryset.values_list('list_of_monitoring_sites', flat=True) for
                        monitoring_site in sublist.split(', '))),
                    'number_of_collected_samples': queryset.aggregate(Sum('number_of_collected_samples'))[
                        'number_of_collected_samples__sum'],
                    'proportion_of_confirmed_samples': (
                            (queryset.aggregate(Sum('nominator_of_confirmed_samples'))[
                                 'nominator_of_confirmed_samples__sum'] or 0) /
                            (queryset.aggregate(Sum('denominator_of_confirmed_samples'))[
                                 'denominator_of_confirmed_samples__sum'] or 0)
                    ) if queryset.aggregate(Sum('denominator_of_confirmed_samples'))[
                        'denominator_of_confirmed_samples__sum'] else 0,
                    'loq_donut': {
                        'data': [
                    ...
                    'property_line': [{"x": str(year["reference_year"]), "y": round(year["avg_value"], 2)} for year in
                                      yearly_data],
                    'property_bar': [{"x": item["observed_property_determinand"], "y": item["total_samples"]} for item
                                     in top_5_observed_determinand],
                    'determinand_table': [
                        {
