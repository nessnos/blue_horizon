...
class GetMapCountries(APIView):
    def get(self, request):
        return Response(
            MapCountrySerializer(CountryStats.objects.values('country', 'iso_alpha2').distinct(), many=True).data)
...
class GetUniqueListData(APIView):
    def post(self, request):
        ...
        if 'observedProperties' in include:
            observed_properties = list(data.values_list('observed_property_determinand', flat=True).order_by(
                'observed_property_determinand').distinct())
