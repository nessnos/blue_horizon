...
class ChatCompletion(APIView):
    def post(self, request):
        ...
        queryset = ChatData.objects.filter(country=country['name']).values('country', 'phenomenon_time_reference_year',
                                                                           'total_monitoring_sites',
                                                                           'water_body_category', 'matrix',
                                                                           'proportion_of_confirmed_samples',
                                                                           'loq_value', 'number_of_collected_samples',
                                                                           'number_samples_below_loq',
                                                                           'mean_concentration',
                                                                           'minimum_recorded_value',
                                                                           'maximum_recorded_value',
                                                                           'unit_of_measure').order_by(
            '-phenomenon_time_reference_year')[:668]
